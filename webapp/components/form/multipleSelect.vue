...
<script lang="ts" setup>
	...
	const selectedOptions = ref(new Array<DataOption>());
	const btnText = ref("");
	...
	const setSelected = (target: HTMLDivElement): void => {
		...
		if (target.classList.contains("selected")) {
			...
			props.data
				.filter((obj) => obj.label === label)
				.map((obj) => selectedOptions.value.push(obj));
			emit("update:modelValue", selectedOptions.value);
		}
		const setText = selectedOptions.value
			.map((obj) => obj.label)
			.join(", ");
		if (setText.length > 0) {
			btnText.value = setText;
			valid.value = true;
			feedback.value = "";
		} else {
	...
	onMounted(() => {
		if (!!props.modelValue.length && props.modelValue.length > 0) {
			btnText.value = props.modelValue.map((obj) => obj.label).join(", ");
			valid.value = true;
			for (let element of options.value) {
				element.classList.remove("selected");
			}
			selectedOptions.value = new Array<DataOption>();
			props.modelValue.map((obj) => {
				options.value.map((element: HTMLDivElement) => {
					if (element.getAttribute("data-label") === obj.label) {
